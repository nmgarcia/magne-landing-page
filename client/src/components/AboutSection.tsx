      ...
      <div className="container mx-auto px-4 sm:px-6 relative z-10 section-content">
        <div className="grid grid-cols-1 lg:grid-cols-2 gap-8 sm:gap-12 lg:gap-16 items-center">
          ...
          <div className="about-content">
            <div className="about-badge inline-flex items-center px-3 sm:px-4 py-2 bg-orange-500/20 border border-orange-500/30 rounded-full mb-4 sm:mb-6">
              <span className="text-xs sm:text-sm font-jetbrains text-orange-500">{t('aboutUs')}</span>
            </div>
            ...
            <div className="space-y-4 sm:space-y-6 mb-6 sm:mb-8">
              <div className="vision-item p-4 sm:p-6 bg-gradient-to-r from-violet-500/10 to-orange-500/10 rounded-xl border border-violet-500/20">
                <h3 className="text-base sm:text-lg font-semibold text-orange-500 mb-2">{t('ourVision')}</h3>
                <p className="text-gray-300 text-sm sm:text-base">{t('visionText')}</p>
              </div>
              ...
              <div className="mission-item p-4 sm:p-6 bg-gradient-to-r from-orange-500/10 to-violet-500/10 rounded-xl border border-orange-500/20">
                <h3 className="text-base sm:text-lg font-semibold text-violet-500 mb-2">{t('ourMission')}</h3>
                <p className="text-gray-300 text-sm sm:text-base">{t('missionText')}</p>
              </div>
            </div>
          ...
          <div className="about-visual relative mt-8 lg:mt-0">
            ...
            <div className="absolute -top-3 sm:-top-6 -right-3 sm:-right-6 bg-slate-800 border border-orange-500/30 rounded-xl p-3 sm:p-4 shadow-2xl">
              <div className="text-lg sm:text-2xl font-bold text-orange-500">5+</div>
            ...
            <div className="absolute -bottom-3 sm:-bottom-6 -left-3 sm:-left-6 bg-slate-800 border border-violet-500/30 rounded-xl p-3 sm:p-4 shadow-2xl">
              <div className="text-lg sm:text-2xl font-bold text-violet-500">50+</div>
              <div className="text-xs sm:text-sm text-gray-300">{t('projectsCompleted')}</div>
            </div>
          </div>
        </div>
        ...
        <div className="mt-16 sm:mt-20 lg:mt-24">
          <div className="text-center mb-8 sm:mb-12 lg:mb-16">
            <h3 className="values-title font-montserrat text-2xl sm:text-3xl lg:text-4xl font-bold mb-3 sm:mb-4">{t('coreValues')}</h3>
            <p className="text-gray-300 max-w-2xl mx-auto text-sm sm:text-base px-4 sm:px-0">{t('valuesDescription')}</p>
          </div>
          ...
          <div className="grid grid-cols-2 sm:grid-cols-3 lg:grid-cols-6 gap-3 sm:gap-4 lg:gap-6">
            {coreValues.map((value, index) => (
              ...
              >
                <div className={`value-icon w-8 sm:w-10 lg:w-12 h-8 sm:h-10 lg:h-12 mx-auto mb-2 sm:mb-3 lg:mb-4 bg-gradient-to-r ${value.gradient} rounded-lg flex items-center justify-center group-hover:scale-110 transition-transform duration-300`}>
                  <i className={`${value.icon} text-white text-sm sm:text-base lg:text-lg`}></i>
                </div>
                <h4 className="font-semibold text-white text-xs sm:text-sm lg:text-base">{value.title}</h4>
              </div>
            ))}
          </div>
        </div>
